library(openxlsx)
#A=read.xlsx("sav2025_algunas_variables.xlsx",sheet = 1)

##Primero las que tiene varias respuestas
A=binary.data2025

N=nrow(A)
#P16 Y, ¿cuál considera que es el principal problema en su colonia?
 #Son 30

unique(A$principal_problema_colonia)
...
#En D están aquellas que no están por arriba de la media
...
################################################################################
#P30A1	Principalmente, ¿qué tipo de delito cree que ocurrió más en Hidalgo en el último año?
...
################################################################################
#P40	¿En dónde o en qué aspecto considera que hay más corrupción?
...
#P12	¿Cuál considera que es el principal problema que enfrenta el estado de Hidalgo?
...
A$principal_problema_estado[A$principal_problema_estado=="ROBOS Y ASALTOS" | A$principal_problema_estado=="FALTA DE VIGILANCIA"]=
  "Inseguridad"

A$principal_problema_estado[A$principal_problema_estado=="POBREZA"]=
  "Economía"

A$principal_problema_estado[A$principal_problema_estado=="DRENAJE" | A$principal_problema_estado=="ESCASEZ DE AGUA"]=
  "Servicios Públicos"

A$principal_problema_estado[A$principal_problema_estado=="NARCOTRÁFICO"]=
  "CRIMEN ORGANIZADO"

A$principal_problema_estado[A$principal_problema_estado=="CONTAMINACIÓN"]=
  "Medio Ambiente"
################################################################################

#Ahora las de pocas respuestas
#P19,P15,
...
Checar=colnames(A)[substr(colnames(A),1,1)=="P" & !(colnames(A) %in% c("P19","P16","P30A1","P40","P99A1","P12","PESOF"))]

for(i in which(colnames(A) %in% Checar)){
  Respuestas= A |>
    dplyr::group_by(.data[[colnames(A)[i]]]) |>
    dplyr::summarise(conteo = dplyr::n())
  D=Respuestas[Respuestas$conteo<6060*0.05,]
  G=Respuestas[Respuestas$conteo>=6060*0.05,]
  if(nrow(D)>0){
    cat("Checar ",colnames(A)[i],"\n")
  }
}

"P15"
Respuestas= A |>
  dplyr::group_by(hogar_recibe_algun_ayuda_del_gobierno ) |>
  dplyr::summarise(conteo = dplyr::n())
D=Respuestas[Respuestas$conteo<N*0.05,]
D[,1]
G=Respuestas[Respuestas$conteo>=N*0.05,]
...
Pro_binary.data2025=A

# Regresion_Pre_2025=glm(formula = reformulate(names(binary.data2025)[2:37],"P19"),
#                        family = binomial(link = "logit"), 
#                        data = binary.data2025,
#                        weights = binary.data2025$PESOF)
# Pre_summary2025=summary(Regresion_Pre_2025)
# Pre_coeficientes_significantes2025=Pre_summary2025[["coefficients"]][Pre_summary2025[["coefficients"]][,4]<0.01,]
# library(DescTools)
# Pre_pseudoR2_2025=PseudoR2(Regresion_Pre_2025, which = c("CoxSnell","Nagelkerke","McFadden"))
# 
# 
# 
# Regresion_Pos_2025=glm(reformulate(names(Pro_binary.data2025)[2:37],"P19"),
#                        family = binomial(link = "logit"), 
#                        data = Pro_binary.data2025, 
#                        weights = Pro_binary.data2025$PESOF)
# Pos_summary2025=summary(Regresion_Pos_2025)
# Pos_coeficientes_significantes2025=Pos_summary2025[["coefficients"]][Pos_summary2025[["coefficients"]][,4]<0.01,]
# Pos_pseudoR2_2025=PseudoR2(Regresion_Pos_2025, which = c("CoxSnell","Nagelkerke","McFadden"))